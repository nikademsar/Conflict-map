...
function popupSection(title, rowsHtml, extraClass = "") {
  if (!rowsHtml) return "";
...
function buildEventPopup(p) {
  ...
  const tovLabel = decode(GED_TYPE_OF_VIOLENCE, p.type_of_violence);

  const d1 = fmtDateISO(p.date_start);
  const d2 = fmtDateISO(p.date_end);
  ...
  const header = `
    <div class="popup-header">
      ...
      <div class="popup-sub">
        <span class="badge badge-tov">${safe(tovLabel)}</span>
      </div>
      ${dateRange ? `<div class="popup-dates">${safe(dateRange)}</div>` : ""}
    </div>
  `;
  ...
  const loc =
    ...
    popupRow("Where", p.where_coordinates) +
    popupRow("Lat, Lon", (p.latitude && p.longitude) ? `${p.latitude}, ${p.longitude}` : "");

  const fat =
    popupRow("Best", p.best) +
    popupRow("Low", p.low) +
    popupRow("High", p.high);

  const ids =
    popupRow("Event", p.id) +
    popupRow("Conflict", p.conflict_new_id ?? p.conflict_id) +
    popupRow("Dyad", p.dyad_new_id);

  return `
    ${header}
    <div class="popup-body">
      ${popupSection("Location", loc)}
      ${popupSection("Fatalities", fat, "fatalities")}

      <details class="popup-details">
        <summary>More details</summary>
        ${popupSection("IDs", ids, "ids")}
      </details>
    </div>
  `;
}
